...
const EnglishCheckerComponent = () => {
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto', padding: '30px', fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif', minHeight: '100vh', backgroundColor: '#f5f5f5' }}>
      ...
      <h3>問題 {currentProblem && <span style={{marginLeft: '10px', padding: '4px 8px', borderRadius: '12px', color:'white', fontSize:'12px', fontWeight:'bold', backgroundColor: currentProblem.level==='beginner'?'#4CAF50':currentProblem.level==='intermediate'?'#FF9800':'#F44336'}}>{currentProblem.level.toUpperCase()}</span>}</h3>
      ...
      {showHints && currentProblem && <div style={{backgroundColor:'#fff3cd', padding:'10px', borderRadius:'5px', marginBottom:'15px', border:'1px solid #ffeaa7'}}><strong>ヒント:</strong> {currentProblem.hints.join(', ')}</div>}
      ...
      <div style={{ display:'flex', gap:'10px', marginBottom:'20px', alignItems:'center' }}>
        <button style={{ background:'linear-gradient(135deg, #667eea 0%, #764ba2 100%)', color:'white', border:'none', padding:'8px 15px', borderRadius:'20px', cursor:'pointer', fontSize:'14px' }} onClick={handleVoiceInput}>🎤 音声入力</button>
        {currentProblem && <button style={{background:'#ffc107', color:'#212529', border:'none', padding:'8px 15px', borderRadius:'20px', cursor:'pointer', fontSize:'14px'}} onClick={()=>setShowHints(!showHints)}>💡 {showHints ? 'ヒントを隠す' : 'ヒントを見る'}</button>}
      </div>
      ...
      <div style={{ display:'flex', gap:'15px', marginBottom:'30px' }}>
        <button
          ...
          onClick={handleCheck} disabled={isChecking}
        >
          {isChecking?'添削中...':'添削する'}
        </button>
        <button style={{ flex:1, padding:'12px 20px', border:'none', borderRadius:'8px', fontSize:'16px', cursor:'pointer', background:'linear-gradient(135deg, #ffecd2 0%, #fcb69f 100%)', color:'#8b4513', fontWeight:'bold' }} onClick={handleNext}>次の問題</button>
      </div>
